...
class DumpToFile(VideoSupplier):
    ...
    def read(self, index, force_type=np):
        ...
        if self.type == "movie":
            ...
            filetype = "png" if out_data.dtype == np.uint8 or out_data.dtype == np.uint16 else "exr"
            ...
            if self.pipe is None:
                ...
                if data.shape[2] == 1:
                    pix_fmt = 'gray8'
                quality = ['-b:v', self.opts.get('bitrate')] if 'bitrate' in self.opts else ['-crf', self.opts.get('crf', 15)]
                command = ["ffmpeg",
